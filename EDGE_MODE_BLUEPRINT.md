...
| Breaking persistence / audit | Truncation runs **after** `maybe_persist_tool_result` |
...
**New helper:** `_edge_mode_truncate_string_tool_result(agent, function_result)`.
...
- For strings longer than **`4000` characters**, replace with **`2000` head + marker + `1500` tail** using module constants:
  - `_EDGE_TRUNC_LEN_TRIGGER = 4000`
  - `_EDGE_TRUNC_HEAD = 2000`
  - `_EDGE_TRUNC_TAIL = 1500`
  - `_EDGE_TRUNC_MARKER` — explicit human/machine visible boundary  
  Character units were chosen for **predictability** independent of tokenizer quirks; the goal is bounding **bytes that hit json serialization and the model API**, not claiming token-exactness.
...
2. `maybe_persist_tool_result(...)` — **full fidelity to disk/sidecar where applicable**
3. **`_edge_mode_truncate_string_tool_result`** — **model-facing shrink**
4. `agent._tool_result_content_for_active_model(...)` → append `messages`

Thus **state and logs can remain complete** while the **active model sequence** is protected.
